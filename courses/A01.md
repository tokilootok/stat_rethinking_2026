...
A CRM simulator might include:

$$
\begin{aligned}
S_i &\sim \text{distribution of price sensitivity}\\
A_i &\sim \pi(A_i \mid S_i,X_i)\\
Y_i &\sim \operatorname{Bernoulli}
\left(
\operatorname{logit}^{-1}
\left[
\alpha+\beta A_i+\gamma S_i+\delta A_iS_i
\right]
\right)
\end{aligned}
$$
...
- $S_i$ is latent price sensitivity;
- $A_i$ is the assigned discount;
- $\pi$ is the historical campaign-assignment policy;
- $Y_i$ is purchase.

If receptive customers are preferentially targeted, $S_i$ affects both treatment and outcome, creating confounding.
