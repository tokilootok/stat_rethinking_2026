...
$$
\widehat{\tau}_{\mathrm{observed}}
=
g(D_{\mathrm{observed}})
$$
...
$$
\begin{aligned}
&\text{Define the scientific target}\\
&\rightarrow \text{model how the data arise}\\
&\rightarrow \text{design an estimator}\\
&\rightarrow \text{test it using simulation}\\
&\rightarrow \text{fit it to observations}\\
&\rightarrow \text{criticize and revise}
\end{aligned}
$$
...
p \mid W,N
\sim
\operatorname{Beta}(\alpha+W,\beta+N-W)
$$
...
$$
\widehat{p}_{\mathrm{Bayes}}
=
\frac{W+\alpha}{N+\alpha+\beta}
$$
...
W^{(s)}
\sim
\operatorname{Binomial}(N,p^\star)
$$
...
- **Historical Treatment Assignment ($A_i$)**: Binary campaign allocation ($A_i = 1$ for €10 discount, $A_i = 0$ for control), governed by historical policy $\pi$:
  ...
  $$\pi_i = \mathbb{P}(A_i = 1 \mid X_i, S_i) = \operatorname{logit}^{-1}\left(\alpha_\pi + \beta_\pi^T X_i + \gamma_\pi S_i\right)$$
- **Outcome ($Y_i$)**: 30-day binary purchase indicator ($Y_i = 1$ if purchase occurs, $0$ otherwise):
  $$Y_i \sim \operatorname{Bernoulli}(p_i)$$
  $$p_i = \operatorname{logit}^{-1}\left(\alpha_Y + \beta_Y A_i + \gamma_Y S_i + \delta_Y (A_i \times S_i) + \eta_Y^T X_i\right)$$
...
The binomial likelihood $W \sim \operatorname{Binomial}(N, p)$ relies on five key assumptions:
...
$$A_i \sim \operatorname{Bernoulli}\left(\pi(A_i = 1 \mid X_i, S_i)\right)$$

If marketers preferentially target receptive or high-intent customers ($\gamma_\pi \neq 0$ in $\pi_i = \operatorname{logit}^{-1}(\alpha_\pi + \beta_\pi^T X_i + \gamma_\pi S_i)$), then $S_i$ influences both treatment assignment $A_i$ and purchase outcome $Y_i$, creating **confounding**. If $S_i$ is unobserved in empirical data, naive comparisons between treated and control customers will be severely biased.
