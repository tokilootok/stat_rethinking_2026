...
$$ \operatorname{logit}^{-1}(\eta) = \frac{\exp(\eta)}{1+\exp(\eta)} $$
...
$$ \operatorname{Var}(Y_i \mid N_i,p_i) = N_i p_i(1-p_i) $$
...
$$ Y_{ig} \sim \operatorname{Bernoulli}(p_g) $$
...
$$ S_g \sim \operatorname{Binomial}(N_g,p_g) $$
...
$$ p(x) = \operatorname{logit}^{-1} \left( \alpha+\beta x \right) $$
...
$$ \frac{ \operatorname{odds}(p_{x+1}) }{ \operatorname{odds}(p_x) } = \exp(\beta) $$

But the corresponding probability difference is:

$$ \Delta p(x) = \operatorname{logit}^{-1} \left( \alpha+\beta(x+1) \right) - \operatorname{logit}^{-1} \left( \alpha+\beta x \right) $$
...
$$ A_{gd} \sim \operatorname{Binomial}(N_{gd},p_{gd}) $$
...
$$ \operatorname{logit}(p_i) = \alpha_{G_i} $$

For grouped data:

$$ A_g \sim \operatorname{Binomial}(N_g,p_g) $$

$$ \operatorname{logit}(p_g) = \alpha_g $$

The probability-scale contrast is:

$$ \Delta_{\mathrm{total}} = \operatorname{logit}^{-1}(\alpha_1) - \operatorname{logit}^{-1}(\alpha_2) $$
...
$$ \operatorname{logit}(p_i) = \alpha_{G_i,D_i} $$

or, in grouped form:

$$ A_{gd} \sim \operatorname{Binomial}(N_{gd},p_{gd}) $$

$$ \operatorname{logit}(p_{gd}) = \alpha_{gd} $$
...
$$ \Delta_d = \operatorname{logit}^{-1}(\alpha_{1d}) - \operatorname{logit}^{-1}(\alpha_{2d}) $$
...
$$ p_{id}^{(s)}(G=1) = \operatorname{logit}^{-1} \left( \alpha_{1,D_i}^{(s)} \right) $$

Predict the same applications as category 2:

$$ p_{id}^{(s)}(G=2) = \operatorname{logit}^{-1} \left( \alpha_{2,D_i}^{(s)} \right) $$
...
$$ \operatorname{logit}^{-1}(\alpha_{1d}) - \operatorname{logit}^{-1}(\alpha_{2d}) $$

These are different summaries.

In particular:

$$ \operatorname{logit}^{-1} \left( \alpha_{1d}-\alpha_{2d} \right) $$
...
$$ p_i^{(s)} = \operatorname{logit}^{-1} \left( \eta_i^{(s)} \right) $$

A replicated binary event is:

$$ Y_i^{\mathrm{rep},(s)} \sim \operatorname{Bernoulli} \left( p_i^{(s)} \right) $$

For grouped trials:

$$ Y_i^{\mathrm{rep},(s)} \sim \operatorname{Binomial} \left( N_i, p_i^{(s)} \right) $$
...
$$ \alpha_c \sim \operatorname{Normal}(3,0.5) $$

$$ \beta_c \sim \operatorname{Normal}(0,0.2) $$
...
$$ P_i = \frac{ \log(\operatorname{population}_i)-\overline{\log P} }{ s_{\log P} } $$
...
$$ P = \frac{ \log(\operatorname{population})-m }{ s } $$

then:

$$ \operatorname{population} = \exp(sP+m) $$
...
$$ T_i^{\mathrm{rep}} \sim \operatorname{Poisson}(\lambda_i) $$
...
$$ \alpha_c \sim \operatorname{Normal}(1,1) $$

$$ \beta_c \sim \operatorname{Exponential}(1) $$
...
$$ Y \Rightarrow \operatorname{Poisson}(\lambda) $$
...
$$ \operatorname{Var}(Y_i \mid X_i) = \mathbb{E}[Y_i \mid X_i] $$
...
$$ p_i^{(s)}(1) = \operatorname{logit}^{-1} \left[ \alpha^{(s)} + \beta_A^{(s)} + X_i^{\mathsf T}\beta_X^{(s)} \right] $$

and control:

$$ p_i^{(s)}(0) = \operatorname{logit}^{-1} \left[ \alpha^{(s)} + X_i^{\mathsf T}\beta_X^{(s)} \right] $$
...
8. Why can $\operatorname{Normal}(0,10)$ be extreme on the probability scale?
