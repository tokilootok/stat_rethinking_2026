...
### 1.2 Where the laboratory and annex fit
...
### 12.1 Purpose: choose the scale that answers the scientific question
...
Using the same two scenarios, define $p_0$ as the starting probability and $p_1$ as the comparison probability:
...
### 12.2 The same odds ratio at four baselines

Continue the illustration $\beta=\log 2$, so every row has odds ratio 2:
...
### 12.3 Translate the contrast into the application

For a campaign with a baseline conversion probability of 10%, the second row describes about 18.18% conversion under the comparison scenario. Over 1,000 comparable opportunities, the model implies about **82 additional expected conversions**, because $1000(0.1818-0.10)\approx82$. That is an expected-count difference, not a guarantee about the realized count.

A useful report would state the two probabilities, the 8.18-percentage-point difference, and its posterior uncertainty. Saying only “odds doubled” leaves the practical magnitude unspecified. For heterogeneous customers, compute their individual probability contrasts and average over a declared target population rather than assuming everyone shares this baseline.
