...
$$ \tau_{\mathrm{total}} = \mathbb{E} \left[ A \mid \operatorname{do}(G=g_1) \right] - \mathbb{E} \left[ A \mid \operatorname{do}(G=g_0) \right] $$
...
$$ \tau_{\mathrm{direct}}(d) = \mathbb{E} \left[ A \mid \operatorname{do}(G=g_1), \operatorname{do}(D=d) \right] - \mathbb{E} \left[ A \mid \operatorname{do}(G=g_0), \operatorname{do}(D=d) \right] $$

A naïve department-specific model is:

$$ \operatorname{logit}(p_i) = a_{G_i,D_i} $$
...
$$ G_i \sim \operatorname{Categorical}(0.5,0.5) $$

The true latent characteristic is binary:

$$ U_i \sim \operatorname{Bernoulli}(0.1) $$
...
$$ u_i \sim \operatorname{Normal}(0,1) $$
...
$$ a_g \sim \operatorname{Normal}(0,1) $$
...
$$ \Delta_p = \operatorname{logit}^{-1}(a_1) - \operatorname{logit}^{-1}(a_2) $$
...
$$ a_{g,d} \sim \operatorname{Normal}(0,1) $$
...
$$ \Delta_{p,d} = \operatorname{logit}^{-1}(a_{1,d}) - \operatorname{logit}^{-1}(a_{2,d}) $$
...
$$ \operatorname{logit}(p_i) = a_{G_i,D_i} + \beta_U U_i $$
...
$$ \operatorname{logit}(p_i) = a_{G_i,D_i} + b_{G_i}u_i $$

### Department model

For a binary department indicator:

$$ D_{2i} \sim \operatorname{Bernoulli}(q_i) $$

$$ \operatorname{logit}(q_i) = \delta_{G_i} + g_{G_i}u_i $$
...
$$ \frac{ \partial \operatorname{logit}(p) }{ \partial u } = b_g $$
...
$$ \frac{ \partial \operatorname{logit}(q) }{ \partial u } = g_g $$
...
$$ b_g \sim \operatorname{Uniform}(0,1) $$

$$ g_g \sim \operatorname{Uniform}(0,1) $$
...
$$ p \left( u,a,\delta,b,g \mid A,D,G \right) \propto p(A \mid G,D,u,a,b) p(D \mid G,u,\delta,g) p(u)p(a)p(\delta)p(b)p(g) $$
...
$$ p_{g,d}^{(s)}(u) = \operatorname{logit}^{-1} \left[ a_{g,d}^{(s)} + b_g^{(s)}u \right] $$
...
$$ p \left( \tau \mid D_{\mathrm{obs}}, b^{(k)}, g^{(k)} \right) $$
...
$$ p(\tau \mid D_{\mathrm{obs}}) = \int p(\tau \mid D_{\mathrm{obs}},b,g) p(b,g \mid D_{\mathrm{obs}}) \,db\,dg $$
...
$$ \Pr \left[ \tau(b,g)>0 \mid D \right] $$

or:

$$ \Pr \left[ |\tau(b,g)|>\tau_{\mathrm{min}} \mid D \right] $$
...
$$ A_{g,d} \sim \operatorname{Binomial}(N_{g,d},p_{g,d}) $$
...
$$ D_i \sim \operatorname{CategoricalLogit}(\theta_{G_i}) $$
...
$$ \operatorname{logit}(p_i) = a_{G_i,D_i} + u_i $$
...
$$ T_{1i} \sim \operatorname{Normal}(u_i,\tau_1) $$

$$ T_{2i} \sim \operatorname{Normal}(u_i,\tau_2) $$

$$ T_{3i} \sim \operatorname{Normal}(u_i,\tau_3) $$

with:

$$ \tau_j \sim \operatorname{Exponential}(1) $$

The admission model remains:

$$ \operatorname{logit}(p_i) = a_{G_i,D_i} + b u_i $$
...
$$ T_{1i} \perp\!\!\!\perp T_{2i} \perp\!\!\!\perp T_{3i} \mid u_i $$
...
$$ \operatorname{logit}(p) = \alpha + \beta_X X + \beta_U U $$

marginalizing over $U$ generally does not yield:

$$ \operatorname{logit} \left[ \mathbb{E}(p \mid X) \right] = \alpha^* + \beta_X X $$
...
$$ \mathcal{S} = \{(b,g): (b,g) \text{ judged plausible}\} $$

the identified sensitivity region is:

$$ \mathcal{T} = \{ \tau(b,g): (b,g) \in \mathcal{S} \} $$
...
$$ D_i^{\mathrm{rep},(s)} \sim \operatorname{Bernoulli} \left( q_i^{(s)} \right) $$

$$ A_i^{\mathrm{rep},(s)} \sim \operatorname{Bernoulli} \left( p_i^{(s)} \right) $$
...
$$ D_i \sim p(D_i \mid G_i,u_i,\delta,g) $$

$$ A_i \sim p(A_i \mid G_i,D_i,u_i,a,b) $$
...
$$ D_i \sim \operatorname{CategoricalLogit} \left( \delta_{G_i} + g_{G_i}u_i \right) $$
...
$$ T_{ji} \sim \operatorname{Normal} \left( \lambda_j u_i, \tau_j \right) $$
...
$$ \tau(b,g) = \mathbb{E} \left[ Y(1)-Y(0) \right] $$
