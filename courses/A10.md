...
$$ G_i \sim \mathrm{Categorical}(0.5,0.5) $$
...
$$ D_i \sim \mathrm{CategoricalLogit}(\theta_{G_i}) $$
...
$$ \mathrm{logit} \left[ \mathbb{E}(p \mid X) \right] = \alpha^* + \beta_X X $$
...
$$ D_i^{\mathrm{rep},(s)} \sim \mathrm{Bernoulli} \left( q_i^{(s)} \right) $$
...
$$ D_i \sim \mathrm{CategoricalLogit} \left( \delta_{G_i} + g_{G_i}u_i \right) $$
